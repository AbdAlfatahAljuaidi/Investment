...
const AddDetail = () => {
  ...
  // ✅ تحديث حالة الإنجاز للتفاصيل
  const handleToggleDone = async (detailId, currentDone) => {
    try {
      await axios.post(
        `${apiUrl}/api/tasks/tasks/${taskId}/details/${detailId}/change`,
        { done: !currentDone },
        { withCredentials: true }
      );
      toast.success('تم تحديث حالة الإنجاز');
      fetchDetails();
    } catch (error) {
